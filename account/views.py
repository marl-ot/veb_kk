...
def account_changes(request):
    ...
    if request.user.is_authenticated:
        ...
        user_info = Auth.objects.filter(id=request.user.id)[0]
        user_school = Schools.objects.filter(id=request.user.school_number_id)[0]
        ...
        if request.user.is_teacher:
            ...
            student_class = Classes.objects.filter(id=request.user.full_class_id)[0]
